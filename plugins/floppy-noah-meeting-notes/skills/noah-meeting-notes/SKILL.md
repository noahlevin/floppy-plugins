...
6. **If Todoist chosen — write idempotently.** First create the grounded audit record, then write tasks with strict dedup so a re-run NEVER creates duplicates (the Todoist-sync-explosion lesson: 4,253 dupes came from skipping dedup).
   ...
   - **Transport:** Todoist has no Cowork connector or MCP today, so writes go via the **Todoist REST API** using a `TODOIST_API_TOKEN` available to this environment.
     ...
     - If it **is** available:
       1. **Dedup (3 layers):** `GET https://api.todoist.com/api/v1/tasks` (filtered to the target project) → collect existing task `content` (titles) and ids. Drop any proposed task whose normalized title already exists. This is the primary guard.
       2. **Create only net-new tasks:** for each remaining task, `POST https://api.todoist.com/api/v1/tasks` with `Authorization: Bearer ${TODOIST_API_TOKEN}`, `Content-Type: application/json`, and a stable **`X-Request-Id: <sha256(draft_id + ":" + normalized_title)>`** header — Todoist treats this as an idempotency key, so a retried POST returns the same task instead of a duplicate. Body: `{ "content", "description", "due_string"?, "project_id"? }`.
